...
export default class RelatedPuppyLogsCmp extends NavigationMixin(
    ...
    switch (actionName) {
      ...
      case "raiser":
        this.navigateToRecord(
          row[RAISER_FIELD.fieldApiName],
          RAISER_FIELD.objectApiName
        );
        break;
      case "dog":
        this.navigateToRecord(
          row[DOG_FIELD.fieldApiName],
          DOG_FIELD.objectApiName
        );
        break;
      case "view":
  ...
  getLogs(result) {
    ...
    if (result.data) {
      this.data = result.data.items.map((info) => {
        ...
        log[ADDITIONAL_FIELD.fieldApiName] = log[
          ADDITIONAL_FIELD.fieldApiName
        ]?.replace(/(<([^>]+)>)/gi, "");
        return log;
      });
  ...
  createLog(event) {
    ...
    this.template.querySelector("c-related-puppy-log-cmp").openModal(payload);
  }
