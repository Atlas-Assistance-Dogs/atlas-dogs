...
import USERTYPE__C_FIELD from '@salesforce/schema/ContentVersion.UserType__c';
...
const COLS = [
    ...
    {label: 'userType', fieldName: 'UserType__c'},
...
export default class ADFileUploadforRelatedList extends LightningElement {
    ...
    @wire(getPicklistValues, { recordTypeId: '012000000000000AAA', fieldApiName: TYPE__C_FIELD })
    picklistValues({ error, data }) {
        if (data) {
           
           this.docPicklistValues = [ { label: '--None--', value: '', selected: true }, ...data.values ];
    ...
    @wire(getPicklistValues, { recordTypeId: '012000000000000AAA', fieldApiName: USERTYPE__C_FIELD })
    picklistValues1({ error, data }) {
        if (data) {
           
           this.userPicklistValues = [ { label: '--None--', value: '', selected: true }, ...data.values ];
