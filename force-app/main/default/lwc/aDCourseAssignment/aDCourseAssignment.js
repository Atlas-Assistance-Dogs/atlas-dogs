...
import updateRecords from '@salesforce/apex/CourseAssigned.updateRecords';
...
export default class ADCourseAssignment extends LightningElement {
    @track error;
    @track columns = COLS;
    @api cmasterlst;
   ...
   // getAssignedCourse({data, error}) {
            ...
            data.forEach((row) => {
               let rowData = {};
                if(localAssignedIds.includes(row.Id)){
                    console.log('inside if of localAssignedIds'+asscoursesObject[row.Id].Status__c );
        ...
        .then(data => {
            let preSelectedLst=data;
                for(let j=0; j<preSelectedLst.length; j++){
                        ...
                        this.CourseNameOutput.push('<br/>'+'Name - '+JSON.stringify(preSelectedLst[j].Course_SIS_Section_SIS_Course_Name__c) );
    ...
    getSelectedName(event) {
        ...
        for (let i = 0; i < selectedRows.length; i++){
            ...
            coursName.push('<br/>'+'Name - '+selectedRows[i].Course_SIS_Section_SIS_Course_Name__c  );
        }
